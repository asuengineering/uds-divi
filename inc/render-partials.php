...
function uds_wp_render_main_nav_menu() {
	...
	if ( 'enabled' === $nav_menu_enabled ) { ?>
		...
		include get_stylesheet_directory() . '/asu-navigation-menu.php';
		?>
	...
	if ( $logo_type && 'asu' === $logo_type ) {
		...
		if ( $logo_select && 'none' !== $logo_select ) {
			...
			echo wp_kses(
				sprintf(
					$logo_template,
					get_stylesheet_directory_uri() . '/src/img/endorsed-logo/' . $filename,
...
function uds_wp_render_asu_footer_logo() {
	...
	echo wp_kses(
		sprintf(
			$logo_template,
			get_stylesheet_directory_uri() . '/src/img/asu-logo/asu_university_horiz_rgb_white_150.png',
			get_bloginfo( 'name' ) . ' Logo',
			'https://asu.edu'
		),
		wp_kses_allowed_html( 'post' )
